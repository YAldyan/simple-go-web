...
func NewImage(user *User) *Image {
	return &Image{
		ID:        util.GenerateID("img", imageIDLength),
		UserID:    user.ID,
		CreatedAt: time.Now(),
	}
}
...
func (image *Image) StaticRoute() string {
	return "/assets/images/" + image.Location
}
...
func (image *Image) StaticThumbnailRoute() string {
	return "/assets/images/thumbnail/" + image.Location
}

func (image *Image) StaticPreviewRoute() string {
	return "/assets/images/preview/" + image.Location
}

/*
	A map of accepted mime types and their file extension

	tambahkan pengecekan, untuk memastikan salah satu file
	yang di-upload, apakah mengandung extension yang eksis
	di bawah ini
*/
...
func (image *Image) CreateFromURL(imageURL string) error {
	...
	if response.StatusCode != http.StatusOK {
		return util.ErrImageURLInvalid
	}
	...
	if err != nil {
		return util.ErrInvalidImageType
	}
	...
	savedFile, err := os.Create("./assets/images/" + image.Location)
	if err != nil {
		return err
	}
...
func (image *Image) CreateResizedImages() error {
	...
	srcImage, err := imaging.Open("./assets/images/" + image.Location)
	if err != nil {
		return err
	}
...
func (image *Image) resizeThumbnail(errorChan chan error, srcImage image.Image) {
	...
	destination := "./assets/images/thumbnail/" + image.Location
...
func (image *Image) resizePreview(errorChan chan error, srcImage image.Image) {
	...
	destination := "./assets/images/preview/" + image.Location
	errorChan <- imaging.Save(dstImage, destination)
}
